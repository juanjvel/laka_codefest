...
Alcance: Fenómeno 1 (IA y Capacidades Estratégicas) únicamente.
Formatos cubiertos: PDF (con OCR de respaldo), JSON (dinámico), CSV (multidelimitador, TSV), Excel, Texto.
"""
...
class CodefestExtractor:
    ...
    def extract_pdf(self, relative_path):
        ...
        # --- MEJORA: OCR de Emergencia para PDFs Escaneados ---
    ...
    def extract_json(self, relative_path):
        ...
        try:
            ...
            # --- MEJORA: Búsqueda dinámica de campos de texto ---
            ...
            # --- MEJORA: Aislamiento riguroso de Metadata ---
            metadata = {
                'url': data.get('url', ''),
...
def ejecutar_pipeline_extraccion(excel_indice_path, base_corpus_dir, output_jsonl_path):
    ...
    df_f1 = df_inventario[df_inventario['Fenómeno'] == FENOMENO_OBJETIVO]
    print(f"Iniciando extracción para {len(df_f1)} documentos de {FENOMENO_OBJETIVO}...")
