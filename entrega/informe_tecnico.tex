...
{\Huge\bfseries\color{azulcf} Documento Técnico}\\[0.4em]
{\Large Etapa 1 --- Construcción de la Base de Conocimiento}\\[1.2em]
{\Large\color{rojocf} CODEFEST AD ASTRA 2026}

\vspace{1.8cm}
\hrule
\vspace{1.8cm}

{\LARGE\bfseries Equipo Laka}

\vspace{1.2em}

{\large
Juan José Vélez Arcila\\[0.3em]
Julián Ojeda Avilés\\[0.3em]
Emanuel Vahos Villa\\[0.3em]
Manuela Galeano Chica
}

\vfill

{\large\bfseries Institución Universitaria ITM}\\[0.5em]
{\large Medellín, Colombia}\\[0.3em]
{\large 13 de agosto de 2026}
...
Somos el \textbf{Equipo Laka}, conformado por Juan José Vélez Arcila, Julián
Ojeda Avilés, Emanuel Vahos Villa y Manuela Galeano Chica, estudiantes de la
\textbf{Institución Universitaria ITM}, en Medellín, Colombia. Participamos
en el CODEFEST AD ASTRA 2026
con este proyecto, que construye la base de conocimiento vectorial de la
Etapa~1 y el módulo de recuperación que la consulta, distribuyendo el trabajo
entre las etapas de extracción y limpieza del corpus, chunking, codificación
e indexación, y el módulo de recuperación que se describe en este documento.

\section{Descripción del problema}

El CODEFEST AD ASTRA 2026 propone el desarrollo de una arquitectura
inteligente multiagente orientada al análisis estratégico de fenómenos con
impacto en el dominio aéreo y espacial, a partir de fuentes abiertas
provenientes de observatorios y centros de pensamiento colombianos,
regionales y globales. El reto se organiza en torno a tres fenómenos: (F1)
inteligencia artificial e innovación en entornos militares, (F2) seguridad
espacial y congestión de la órbita baja terrestre, y (F3) dinámicas
territoriales en América Latina con implicaciones de seguridad y
gobernanza. La Etapa~1, que es la fase virtual del reto y la que atañe a
este documento, exige construir una \textbf{base de conocimiento vectorial}
a partir del corpus documental provisto (PDF, HTML, JSON, CSV/XLSX,
imágenes y mapas \codigo{.pbf}) y evaluarla exclusivamente por la
\textbf{calidad de recuperación}: qué tan relevantes son los documentos y
fragmentos que el sistema retorna ante 50 consultas en lenguaje natural.
Esta base servirá como infraestructura para un asistente conversacional
en la Etapa~2 (presencial), pero en esta etapa el criterio de evaluación
es únicamente la recuperación de información, sin intervención de modelos
generativos en ningún punto del proceso.
...
\subsection{Observaciones sobre la salida y correcciones aplicadas}
\label{sec:observaciones}

La entrega original de la Etapa~1 (13 de agosto de 2026) presentaba dos
comportamientos medidos directamente sobre \codigo{resultados.jsonl}. Tras la
clasificación a la Etapa~2, con margen para revisar la base antes del
asistente conversacional, ambos se corrigieron en
\codigo{entrega/generador.py} y se verificaron de nuevo sobre las 50
consultas reales contra el mismo índice FAISS:

\begin{itemize}
\item \textbf{Normalización de texto (corregido)}: el 89\,\% de los
fragmentos (445/500) arrastraba saltos de línea de la extracción de PDF a
mitad de frase, mientras que el 100\,\% de los excertos anotados en el
ground truth de referencia son de una sola línea. Como la relevancia se
juzga por el \emph{contenido} del campo \codigo{text} (Sección~10.2.1), se
implementó \codigo{normalizar\_texto()}, que colapsa saltos de línea y
espacios repetidos antes de escribir cada fragmento --- sin infringir el
esquema (la Sección~9.2.1 ya autoriza modificar el texto de salida) y sin
tocar el campo \codigo{texto} de \codigo{metadata.jsonl}, que la Tabla~1
exige sin modificaciones. Verificado sobre las 50 consultas reales:
\textbf{445/500 (89.0\,\%) $\to$ 0/500 (0.0\,\%)}.
\item \textbf{Deduplicación de fragmentos (corregido)}: al no filtrar por
similitud, 19 de las 50 consultas (38\,\%) tenían al menos dos fragmentos
casi idénticos dentro de su propio top-10 --- esperable dado que el corpus
incluye documentos con filas de contenido muy repetitivo. Se implementó
\codigo{es\_casi\_duplicado()} (similitud de secuencia de caracteres sobre
el texto ya normalizado, umbral 0.92), integrada en
\codigo{construir\_fragmentos()}: un candidato casi idéntico a uno ya
aceptado se descarta y el bucle de expansión de $k$ (Sección~8.6) sigue
recuperando candidatos hasta completar el top-10 con contenido distinto.
Autorizado por la Sección~8.7 (post-filtro por contenido de metadata).
Verificado sobre las 50 consultas reales: \textbf{19/50 (38.0\,\%) $\to$
0/50 (0.0\,\%)}.
\end{itemize}

Ambas correcciones se validaron por reproducibilidad exacta: regenerar
\codigo{resultados.jsonl} desde cero con \codigo{generador.py} produce las
50 consultas idénticas byte a byte al archivo entregado.

\subsection{Contrato de invocación}

Conforme a la Sección~1.5, \codigo{generador.py} se ejecuta con
\codigo{python generador.py} sin argumentos desde la raíz de
\codigo{entrega/}: \codigo{--indice} y \codigo{--consultas} tienen valores
por defecto que coinciden con la estructura entregada
(\codigo{base\_vectorial/encoder\_multilingual-e5-small} y
\codigo{consultas.jsonl}, respectivamente, siguiendo el criterio de la
Tabla~1 del spec), y producen \codigo{resultados.jsonl} de forma
reproducible.
...
Como componente bonus se construyó un grafo de conocimiento
(\texttt{base\_vectorial/grafo/grafo.graphml}, formato GraphML, 452{,}379 nodos y
1{,}155{,}098 aristas) a partir de los mismos chunks indexados. Los nodos son
entidades nombradas (tipos \texttt{PER}, \texttt{ORG}, \texttt{LOC}) con
atributos de frecuencia y fenómeno(s) (\texttt{1}/\texttt{2}/\texttt{3}) en
que aparecen; las aristas son relaciones de coocurrencia
(\texttt{co\_occurs\_with}) ponderadas por número de coocurrencias, y
conservan trazabilidad completa hacia \texttt{doc\_ids} y \texttt{chunk\_ids}
de origen, más un ejemplo de evidencia textual cuando aplica. El grafo no
participa en la recuperación en esta entrega (que sigue siendo puramente
vectorial vía FAISS, sin LLMs) y se entrega como artefacto exploratorio
complementario para análisis de co-ocurrencia de entidades entre fenómenos.

El archivo es XML bien formado y carga sin errores con
\texttt{networkx.read\_graphml} u otra herramienta compatible (Gephi, yEd,
Neo4j): sin caracteres de control ilegales, sin ids de nodo duplicados y sin
nodos vacíos.
